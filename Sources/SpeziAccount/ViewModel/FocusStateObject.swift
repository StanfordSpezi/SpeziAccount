...
class FocusStateObject: ObservableObject {
    ...
    let focusedField: FocusState<String?> // see `AccountKey.Type/focusState`


    /// Initializes a new FocusStateObject object.
    /// - Parameters:
    ///   - focusedField: The `FocusState` of the data entry view.
    init(focusedField: FocusState<String?>) {
        self.focusedField = focusedField
    }

    subscript<Member>(dynamicMember keyPath: KeyPath<FocusState<String?>, Member>) -> Member {
        focusedField[keyPath: keyPath]
    }

    subscript<Member>(dynamicMember keyPath: ReferenceWritableKeyPath<FocusState<String?>, Member>) -> Member {
        get {
            self[dynamicMember: keyPath as KeyPath<FocusState<String?>, Member>]
        }
        set {
